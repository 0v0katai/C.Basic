...
int MatOprand( unsigned char *SRC, int *reg){ 
	...
	if ( ( 'A'<=c )&&( c<='Z' ) ) {
		(*reg)=c-'A';
		ExecPtr++ ;
		if ( SRC[ExecPtr] != '[' ) { ErrorNo=SyntaxERR; ErrorPtr=ExecPtr; return -1; }	// Syntax error
		c=SRC[++ExecPtr];
		if ( SRC[ExecPtr+1] == ',' ) {
			ExecPtr++ ;
			if  ( ( '1'<= c ) && ( c<='9' ) ) dimA=c-'0';
		...
		if ( MatArySizeA[(*reg)] < dimA ) { ErrorNo=DimensionERR; ErrorPtr=ExecPtr; return -1; }	// Dimension error
		c=SRC[++ExecPtr];
		if ( SRC[ExecPtr+1] == ']' ) {
			ExecPtr++ ;
			if  ( ( '1'<= c ) && ( c<='9' ) ) dimB=c-'0';
		...
		if ( MatArySizeB[(*reg)] < dimB ) { ErrorNo=DimensionERR; ErrorPtr=ExecPtr; return -1; }	// Dimension error
		ExecPtr++ ;
		return (dimA-1)*MatArySizeB[(*reg)]+dimB-1;
	} else
	{ ErrorNo=SyntaxERR; ErrorPtr=ExecPtr; return -1; }	// Syntax error
	return -1; //
}
//-----------------------------------------------------------------------------
...
int StoPict( int pictNo){
	...
	if ( stat != 0 ) { ErrorNo=MemoryERR; ErrorPtr=ExecPtr; return; }	// Memory error
	return stat;
}
...
double CB_BinaryEval( unsigned char *SRC ) {
	...
	if ( c==0x7F ) {
		...
		if ( c == 0x40 ) {	// Mat A[a,b]
			...
			mptr=MatOprand( SRC, &reg);
			if ( ErrorNo ) return ; // error
			src = MatAry[reg][mptr];			// Matrix array
		}
	...
	if ( c==0x7F ) {
		...
		if ( c == 0x40 ) {	// Mat A[a,b]
			...
			mptr=MatOprand( SRC, &reg);
			if ( ErrorNo ) return ; // error
			dst = MatAry[reg][mptr];			// Matrix array
		}
...
//	ExecPtr++;
	switch ( op ) {
			...
			case 0xA9 :		// ~
				return src * dst;
			case 0xB9 :		// €
				if ( dst == 0 ) { ErrorNo=MaERR; ErrorPtr=opPtr+1; return; }  // Math error
				return src / dst;
			case '=' :	// =
				return src == dst;
			case '>' :	// >
...
double CB_UnaryEval( unsigned char *SRC ) {
	...
	if ( c==0x7F ) {
		...
		if ( c == 0x40 ) {	// Mat A[a,b]
			...
			mptr=MatOprand( SRC, &reg);
			if ( ErrorNo ) return ; // error
			return MatAry[reg][mptr];			// Matrix array
		}
