...
unsigned short CB_FrameColor=0xFFFF;			// current frame color (default White)
...
void CB_AliasVar( char *SRC ) {	// Alias A=ƒ¿  or A=_ABCD	// (delete) Alias -A=_ABCD  Alias Mat -A=_ABCD
	...
	if ( ( c==0x7F ) && ( ( d==0x40 ) || ( d==0xFFFFFF84 ) ) ) {	// 	Alias Mat/Vct A=ƒ¿  or Mat/Vct A=_ABCD 
		ExecPtr+=2;
		c=SRC[ExecPtr];
	...
	if ( ( c==0xFFFFFFE2 ) ) {	// 	Alias Lbl A=ƒ¿  or Lbl A=_ABCD
		ExecPtr++;
		c=SRC[ExecPtr];
		if ( CB_CheckAliasVar( c, &del ) ) return ;	// Syntax error
		reg=RegVar(c);
