...
int OpcodeToText( char *srcbase, char *text, int maxsize ) {
	...
	int IsGB;
	int	maxtextsize=SrcSize( srcbase-0x56 )-1;
	int progressofst=0;
	...
	if ( EditGBFont ) {
		text[textofst-5] ='G';
		text[textofst-4] ='B';
		text[textofst-3] ='0'+EditGBFont;
	}
	GBcode = EditGBFont;
	while ( cont ) {
	  tokenloop:
//		if ( ofst >= progressofst ) {
			ProgressBar( ofst, maxtextsize ) ;	// ProgressBar 
//			progressofst+=maxtextsize/4096;
//			if ( progressofst > maxtextsize )  progressofst=maxtextsize;
//		}
		code = GetOpcodeGB( srcbase, ofst ) ;
		if ( OpcodeLenGB( code ) == 4 ) {	// 0x5Cxx5Cxx
				if ( flag ) text[textofst++] ='_';
				text[textofst++] ='#';
				NumToHex( text+textofst, code>>16 , 4);
				textofst+=4;
				ofst+=2;
				if ( flag ) text[textofst++] ='_';
				code = code &0xFFFF;
		...
		if ( (GBcode!=0) && ( (code>>8) == 0x5C ) ) {
			goto code4;
		} else
		if ( IsGBCode( code ) ) {
			if ( ( DirectGBconvert==0 ) || (code==0x03A8) ) { GBcode = 2;  goto code4; }
			text[textofst++] = (code>>8)&0xFF;
		...
		if ( ( code == 0x0D ) && ( flag == 0x27 ) ) { flag=0; GBcode = EditGBFont; }	// ' end
		if ( ( code == 0x0D ) && ( flag == 0x22 ) && ( quotflag == 0 ) ) { flag=0; GBcode = EditGBFont; }	// " end
		if ( code == 0x22 ) {	// "
			if ( flag == 0x22 ) { { flag=0; GBcode = EditGBFont; } // " end
			} else {
				flag=0x22;	// "
				c=srcbase[ofst-1];
		...
		if ( ( 0xE500 <= code ) && ( code <= 0xE6FF ) ) {	// 2byte
			...
			if ( ( 0x0001 <= code ) && ( code <= 0x002F ) ) opstr=(char*)ConvList0000[code];
			else
			if ( ( code == 0x0090 ) ) opstr="X";
			else
			if ( ( 0x0080 <= code ) && ( code <= 0x00FE ) ) opstr=(char*)ConvList0080[code-0x0080];
			else
			if ( ( 0x7F00 <= code ) && ( code <= 0x7FFF ) ) opstr=(char*)ConvList7F00[code-0x7F00];
			else
			if ( ( 0xF700 <= code ) && ( code <= 0xF7FF ) ) opstr=(char*)ConvListF700[code-0xF700];
			else
			if ( ( 0xF900 <= code ) && ( code <= 0xF9FF ) ) opstr=(char*)ConvListF900[code-0xF900];
			else
			if ( ( 0xE700 <= code ) && ( code <= 0xE7FF ) ) opstr=(char*)ConvListE700[code-0xE700];
			else {	// other code
				text[textofst++] ='@';
				NumToHex( text+textofst, code, 4);
				textofst+=4;
			...
			if ( ( flag ) && ( ( (code<0x20) && (code!=0x0D) ) || ( (0x80<=code) ) && ( (code!=0x89) && (code!=0x99) && (code!=0xA8) && (code!=0xA9) && (code!=0xB9) && (code!=0xAB) ) ) ) {
				text[textofst++] ='_';
				strncpy( text+textofst, opstr, len );
				textofst += len;
				ofst += OpcodeLenGB( code );
				text[textofst++] ='_';
			} else {
				strncpy( text+textofst, opstr, len );
				textofst += len;
		...
		if ( code == 0x0C ) { // Disps
			{ flag=0; GBcode = EditGBFont; }
//			if ( GetOpcodeGB( srcbase, ofst ) != 0x0D ) {;
				text[textofst++] ='\r';
				text[textofst++] ='\n';
//			}
		}
...
int ex_codecnv( char *srcbase, char *text, int *ofst, int *textofst ) {
	...
	i=0;
	do {
		opstr=(char*)ConvOpCodeStrList[i].str;
...
int TextToOpcode( char *filebase, char *text, int maxsize ) {
	...
	if ( strncmp( text, cbasicstr, 13 ) == 0 ) { textofst+=18;
		c = text[textofst-3];
		if ( c=='1' ) {
			GBconvertMode=1;
			GBcode=1;
	...
	while ( cont ) {
	  tokenloop:
		...
		if ( c == '0' ) {	// '	0X.... 0x... 0B.... 0b....
			d = text[textofst+1];
			if ( ( d=='X' )||( d=='x' )||( d=='B' )||( d=='b' )||( d==0xFFFFFF90 ) ) {
				srcbase[ofst++] = c;
				textofst++;
				len=64;
				GetVarName( text, &textofst, srcbase+ofst, &len) ;
		...
		if ( c == '#' ) {
				...
				if ( ( (0xA001<=d) && (d<=0xFFFE) ) || ( (0x5C01<=d) && (d<=0x5CFE) ) || ( (0x03A8==d) ) ) {
					srcbase[ofst++] = d >> 8;
					srcbase[ofst++] = d & 0xFF;
				} else {
					srcbase[ofst++] = c;
	  ...
	  tokenskip:
		if ( ( 0x20 <= c ) && ( c <= 0x7E ) ) {
			if ( c == 'C' ) { 	// Char
				...
				if ( c == 'X' ) if ( XInputMethod==0 ) c=0x90;
				d=text[textofst+1];
				n=text[textofst+2];
				if ( (
				 ( ('a'<=c)&&(c<='c')&&('0'<=n)&&(n<='2') ) ||
		...
		if ( ( 0xFFFFFFA0 <= c ) && ( c <= 0xFFFFFFDF ) ) {	// kana
				if ( DirectGBconvert==0 ) srcbase[ofst++] = 0xFF;
				srcbase[ofst++] = c;
				textofst++;
		} else {	// no token
				srcbase[ofst++] = '?';
