...
//	if ( x >= 0 ) return floor(x+5e-15);
//	return floor(x-5e-15);
//}
...
double Eval_atof(char *SRC, int c) {
	...
	int d;
		if ( c == '.'  )   {	// .123456
			...
			if ( c == '0' ) {
				d = SRC[ExecPtr+1];
				if ( ( (  'B'<=d ) && ( d<='x' ) ) || ( d==0xFFFFFF90 ) ) return Eval_atoi( SRC, c );
			}
