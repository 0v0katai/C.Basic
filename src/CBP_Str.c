...
int StrGetOpcode( char *SRC, int ptr ){
  ...
  start:
	...
	switch ( c ) {
		...
		case 0x5C:		// backslash
			c=SRC[ptr+1]&0xFF;
			if ( (((0xE5<=c)&&(c<=0xE7))||(c==0x7F))&&(SRC[ptr+2]==0x5C) ) {	// escape GB code
				return ( 0x5C005C00 + (c<<16) + (unsigned char)SRC[ptr+3] );	// escape GB code
			} else 0x5C00 + c;
			break;
		case 0x7F:		// 
		case 0xFFFFFFF7:		// 
		...
		default:
			if ( GBcode ) { 
				d=SRC[ptr+1];
				if ( (c==0x03)&&(d==0xFFFFFFA8) ) { ptr+=2; GBcode=2; goto start; }	// GB code pre byte
				if ( IsGBCode2(c,d) ) goto code2;	// GB code
			}
	}
  ...
  start:
	...
	switch ( c ) {
		...
		case 0x5C:		// backslash
			...
			if ( (((0xE5<=c)&&(c<=0xE7))||(c==0x7F))&&(SRC[(*ptr)]==0x5C) ) {	// escape GB code
				(*ptr)++;
				return ( 0x5C005C00 + (c<<16) + (unsigned char)SRC[(*ptr)++] );	// escape GB code
			} else return 0x5C00 + c;
			break;
		case 0x7F:		// 
		case 0xFFFFFFF7:		// 
		case 0xFFFFFFF9:		// 
		...
		default:
			if ( GBcode ) { 
				d=SRC[(*ptr)];
				if ( (c==0x03)&&(d==0xFFFFFFA8) ) { (*ptr)++; GBcode=2; goto start; }	// GB code pre byte
				if ( IsGBCode2(c,d) ) goto code2;	// GB code
			}
	}
	return (unsigned char)c ;
...
void StrPutOpcodeInc( char *SRC, int *ptr, int opcode ){
	if ( opcode >= 0x1000000 ) {
		SRC[(*ptr)++]=(opcode>>24)&0xFF;
		SRC[(*ptr)++]=(opcode>>16)&0xFF;
		SRC[(*ptr)++]=(opcode>> 8);
		SRC[(*ptr)++]=(opcode    )&0xFF;
	} else
	if ( opcode >= 0x100 ) {
...
//#define StrPrevOpcode PrevOpcodeGB
//#define StrNextOpcode NextOpcodeGB
		...
		switch ( c ) {
			...
			case 0xFFFFFFFF:	// 
			  code2:
				...
				break;
			default:
				if ( GBcode ) if ( IsGBCode2(c,SRC[srcPtr]) ) goto code2;	// GB code
...
void OpcodeStringToAsciiString(char *buffer, char *SRC, int Maxlen ) {	// Opcode String  ->  Ascii String
	...
	int c=1,d;
	while ( c != '\0' ) {
		c = StrGetOpcodeInc( SRC, &srcPtr );
		if ( (c>>8)==0x5C ) {
			if ( (GBcode!=0) && CheckEscape1(c&0xFF) )  CB_OpcodeToStr( c, tmpbuf ) ;	// SYSCALL
			else {
				tmpbuf[0]= c&0xFF;	// 
				tmpbuf[1]='\0';
			}
...
void OpcodeStringToAsciiString_noESC(char *buffer, char *SRC, int Maxlen ) {	// Opcode String  ->  Ascii String no ESCAPE
	char tmpbuf[18];
	int i,j=0,len,srcPtr=0,ptr=0;
	int c=1,d;
	while ( c != '\0' ) {
...
void CB_GetFileStr(char *SRC, char *buffer, int Maxlen ) {
	...
	OpcodeStringToAsciiString_noESC( buffer, buffer2, Maxlen );
}
...
//	HiddenRAM_freeProg( CB_StrBufPtr );
}
char* NewStrBuffer(){
	int reg=Mattmp_strBuffer;
	CB_StrBufferCNT++;
	if ( CB_StrBufferCNT > CB_StrBufferCNTMax ) { CB_Error(MemoryERR); return NULL; }	// Memory error
	if ( CB_StrBufPtr==NULL ) {
//		CB_StrBufPtr = CB_StrBuf;
		CB_StrBufPtr = (char*)HiddenRAM_mallocProg( CB_StrBufferCNTMax * CB_StrBufferMax );
		if ( CB_StrBufPtr==NULL ) { CB_Error(MemoryERR); return NULL; }	// Memory error
	}
	if ( CB_StrBufferCNT < 0 ) { CB_Error(MemoryERR); return NULL; }	// Memory error
	return CB_StrBufPtr + (CB_StrBufferCNT-1)*CB_StrBufferMax;
}
/*
void DeleteStrBuffer(){
	int i;
...
//	CB_StrBufPtr = (char*)MatAry[reg].Adrs;
...
//	if ( ( buffer[4] != '/' ) || ( buffer[7] != '/' ) ) { CB_Error(SyntaxERR); return ; }  // Syntax error
	RTC_GetDateTime( timestr );
	timestr[0]=((buffer[0]-'0')<<4) + buffer[1]-'0';	// year1
	timestr[1]=((buffer[2]-'0')<<4) + buffer[3]-'0';	// year1
	timestr[2]=((buffer[5]-'0')<<4) + buffer[6]-'0';	// min
	timestr[3]=((buffer[8]-'0')<<4) + buffer[9]-'0';	// sec
//	timestr[4]=hour;
//	timestr[5]=min;
//	timestr[6]=sec;
	timestr[7]=0;
...
void ClrLine5800P( int CsrX ){
	int d;
	if ( CsrX > 21 ) return ;
	if ( CB_fx5800P == 0 ) for( d=(CsrX-1)*18; d<384; d+=18 ) CB_PrintModeC( d, (CursorY-1)*24, (unsigned char *)" ", MINI_OVER );	
}
void CB_StrPrints( char *SRC , int csrX ) {
	char buffer[256];
	int c,d,i=0;
	int px,code;
	...
	if ( c == 0x0E ) {	// -> store str
		...
		if ( ( CursorX >1 ) ) Scrl_Y();
		CursorX+=csrX; px=(CursorX-1)*18;
		while ( buffer[i] ) {
			if ( ( CursorX > 21 ) ) { Scrl_Y(); px=0; }
			code = GetOpcodeGB( buffer, i );
			d = CB_PrintModeC( px, (CursorY-1)*24, (unsigned char *)(buffer+i), MINI_OVER | extAnkfont );	// extflag
			px += d;
			CursorX = ((px+17)/18)+1;			
			c = buffer[i] ;
...
void CB_Str( char *SRC ){		// "" "" or $Mat or Str1-20 or StrFunction
	...
	if ( ErrorNo == 0 ) CB_StrPrints(SRC, 0);
	GBcode=bk_GBcode;
}
int CB_StrGB( char *SRC ){		// #"" "" or $Mat or Str1-20 or StrFunction for GB code
	int maxoplen;
	int bk_GBcode=GBcode;
	int c=CB_IsStr( SRC, ExecPtr );
	if ( c==0 ) return c;
	GBcode=2;	// full Gb code enable
	CB_CurrentStr = CB_GetOpStr( SRC, &maxoplen );	
	if ( ErrorNo == 0 ) CB_StrPrints(SRC, 0);
	if ( SRC[ExecPtr]=='?' ) {
		ExecPtr++;
...
int CB_StrAsc( char *SRC ) {	// StrAsc("A")  -> 0x41
	...
	if ( ErrorNo ) return 0;  // error
	if ( SRC[ExecPtr] == ')' ) ExecPtr++;
	OpcodeStringToAsciiString( buffer2, buffer, 32-1 );
	return GetOpcodeGB( buffer, 0 );
//	return StrGetOpcode( buffer2, 0 );
}
