...
void CB_GetOprand2VW( char *SRC, int *px, int *py) {
	double x,y;
	if ( (unsigned char)SRC[ExecPtr] == '#' ) {	// ViewWwindow(x,y,x2,y2)->(px,py,px2,py2)
		ExecPtr++;
...
void CB_GetOprand3VWyxx( char *SRC, int *py, int *px1, int *px2) {
	double y,x1,x2;
	if ( (unsigned char)SRC[ExecPtr] == '#' ) {	// ViewWwindow(y,x1,x2)->(py,px,px2)
		ExecPtr++;
		y=CB_EvalDbl( SRC );
		if ( (unsigned char)SRC[ExecPtr] != ',' ) { CB_Error(SyntaxERR); return; }  // Syntax error
		ExecPtr++;
...
void CB_GetOprand3VWxyy( char *SRC, int *px, int *py1, int *py2) {
	double x,y1,y2;
	if ( (unsigned char)SRC[ExecPtr] == '#' ) {	// ViewWwindow(x,y1,y2)->(px,py,py2)
		ExecPtr++;
		x=CB_EvalDbl( SRC );
		if ( (unsigned char)SRC[ExecPtr] != ',' ) { CB_Error(SyntaxERR); return; }  // Syntax error
		ExecPtr++;
...
int CB_GetOprand_percent( char *SRC ) {	// 0~100
	...
	int c;
	int value=100;
	if ( (unsigned char)SRC[ExecPtr] != ',' ) return value;	// no rand oprand
	ExecPtr++;
...
void CB_GetOprand_MLcolor( char *SRC, int *color) {
	...
	*color=ML_BLACK;
	MLV_rand=(32767+1);	//
	if ( (unsigned char)SRC[ExecPtr] != ',' ) return;	// no oprand
	ExecPtr++;
	if ( (unsigned char)SRC[ExecPtr] == ',' ) goto jp;
	*color=CB_EvalInt( SRC );
  jp:
	if ( (unsigned char)SRC[ExecPtr] != ',' ) return;	// no rand oprand
	ExecPtr++;
	c = (unsigned char)SRC[ExecPtr];
	if ( c == ',' ) return;
	if ( c == '%') { ExecPtr++;
...
unsigned char* CB_ML_GetMatAdrs( char *SRC ) {
	int reg,x,y;
	int c = (unsigned char)SRC[ExecPtr];
	if ( ( c=='&' ) || ( ( c==0x7F)&&(SRC[ExecPtr+1]==0x000000F8) ) {	// & or VarPtr(
...
int CB_MLTest( char *SRC) { // MLTest_command
	int c;
	c = (unsigned char)SRC[ExecPtr];
	if ( c != 0x000000F9 ) { CB_Error(SyntaxERR); return 0; }  // Syntax error
	ExecPtr++;
	c = (unsigned char)SRC[ExecPtr++];
	switch ( c ) {
		case 0x000000C5:	// _Point
			return CB_MLTest_Point( SRC );
		case 0x000000C7:	// _Line
			return CB_MLTest_Line( SRC );
		case 0x000000C8:	// _Horizontal
			return CB_MLTest_Horizontal( SRC );
		case 0x000000C9:	// _Vertical
			return CB_MLTest_Vertical( SRC );
		case 0x000000CA:	// _Rectangle
			return CB_MLTest_Rect( SRC );
		case 0x000000CB:	// _Polygon
		case 0x000000CC:	// _FillPolygon
			return CB_MLTest_Polygon( SRC );
		case 0x000000CD:	// _Circle
		case 0x000000CE:	// _FillCircle
			return CB_MLTest_Circle( SRC );
		case 0x000000CF:	// _Elips
		case 0x000000D0:	// _FillElips
			return CB_MLTest_Ellipse( SRC );
		case 0x000000D1:	// _ElipsInRct
		case 0x000000D2:	// _FElipsInRct
...
void CB_ML_command( char *SRC, int c ) { // ML_command
	switch ( c ) {
		...
		case 0x000000D2:	// _FElipsInRct
			...
			break;
		case 0x000000D5:	// _Bmp
			CB_ML_Bmp( SRC );
			break;
		case 0x000000D6:	// _Bmp8
			CB_ML_Bmp8( SRC );
			break;
		case 0x000000D7:	// _Bmp16
			CB_ML_Bmp16( SRC );
			break;
		case 0x000000D9:	// _BmpZoom
			CB_ML_BmpZoom( SRC );
			break;
		case 0x000000DA:	// _BmpRotate
			CB_ML_BmpRotate( SRC );
			break;
		case 0x000000DE:	// _BmpZoomRotate
			CB_ML_BmpZoomRotate( SRC );
			break;
			
		case 0x000000C0:	// _ClrVRAM
//			CB_ML_ClrVRAM();
			ML_clear_vram();
			break;
		case 0x000000C1:	// _ClrScreen
//			CB_ML_ClrScreen();
			if ( Is35E2 == 0 ) ML_clear_screen();
			break;
		case 0x000000C2:	// _DispVRAM
			CB_ML_DispVRAM( SRC );
//			ML_display_vram();
			break;
		case 0x000000C3:	// _Contrast
			CB_ML_SetContrast( SRC );
			break;
		case 0x000000D3:	// _Hscroll
			CB_ML_H_Scroll( SRC );
			break;
		case 0x000000D4:	// _Vscroll
			CB_ML_V_Scroll( SRC );
			break;
		case 0x000000DB:	// BmpSave
			CB_BmpSave( SRC );
			break;
		case 0x000000DC:	// BmpLoad
			CB_BmpLoad( SRC );
			break;
		case 0x000000DD:	// DrawMat
			CB_DrawMat( SRC );
			break;
	}
}
