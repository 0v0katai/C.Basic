		...
		if ( c == 0x40 ) {	// Mat A[a,b]
		  ...
		  Listj:
			if ( SRC[ExecPtr] != '[' ) { 
				...
				if ( CBint_CurrentValue == 0 ) { CB_Error(OutOfDomainERR); return ; } // // Out of Domain error
...
int RangeErrorCKint( char *SRC ) {
	...
	if ( ( Xdot == 0 ) || ( Ydot == 0 )  ) { ErrorNo=RangeERR; PrevOpcodeGB( SRC, &ExecPtr ); ErrorPtr=ExecPtr; return ErrorNo; }	// Range error
	return 0;
}
*/
//----------------------------------------------------------------------------------------------
...
//	if ( ( py<MatBase ) || ( py>62*CB_G1MorG3M+1 ) )  {  CB_Error(OutOfDomainERR); return ; } // Out of Domain error
	if ( ( py<MatBase ) || ( py>62*CB_G1MorG3M+1 ) )  {  err=1; } // Out of Domain error
	if ( SRC[ExecPtr] != ',' ) { CB_Error(SyntaxERR); return; }  // Syntax error
	ExecPtr++;
	px = (EvalIntsubTop( SRC ));
//	if ( ( px<MatBase ) || ( px>126*CB_G1MorG3M+1 ) )  {  CB_Error(OutOfDomainERR); return ; } // Out of Domain error
	if ( ( px<MatBase ) || ( px>126*CB_G1MorG3M+1 ) )  {  err=1; } // Out of Domain error
...
int CB_MandelInt( int N, int c, int d, int k, int multi ){
	int tmpa,a=0,b=0,i=0,multi2=multi/2;
	while ( (i<N) && ( (a*a+b*b)<k ) ) {
		i++;
		tmpa=a;
		a=(a*a-b*b)/multi+c;
...
int CB_MandelInt2( int N, int c, int d, int k, int multi ){
	int tmpa,a=0,b=0,i=0,multi2=multi/2;
	while ( (i<N) && ( ((a*a+b*b)/multi)<k ) ) {
		i++;
		tmpa=a;
		a=(a*a-b*b)/multi+c;
		b=(tmpa*b)/multi2 +d;
...
int CB_MandelDbl( int N, double c, double d, double k){
	double tmpa,a=0,b=0;;
	int i=0;
	while ( (i<N) && ( (a*a+b*b)<k ) ) {
		i++;
...
int CB_SysCall( char *SRC ) {	// SysCall( No,r4,r5,r6,r7 )
	...
	if ( callNo == 10000 ) {
		...
		r6d=*((double*)r[2]);
		r7d=*((double*)r[3]);
		return CB_MandelDbl( r[0],r5d,r6d,r7d );
	} else
	if ( callNo == 10001 ) return CB_MandelInt( r[0],r[1],r[2],r[3],r[4] );
	else
	if ( callNo == 10002 ) return CB_MandelInt2( r[0],r[1],r[2],r[3],r[4] );
